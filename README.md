...
**画面が真っ白のとき**: 基板は正しく認識され、診断ファームでも白い場合は、パネル / IO エキスパンダ / 電源 IC の状態が
固まっている (ESP のリセットでは戻らない)。**電源ボタンを 6〜10 秒長押しして電源を切り、もう一度押して起動**すると戻る
(実際に起きた。原因の候補は IO エキスパンダのプルアップ設定 (0x09) を誤って 0 にしたこと。現在は起動時に復元している)。
起動ログの `[ioe1] before:` 行に IO エキスパンダのレジスタ 0x00..0x27 が出る (0x03 MODE, 0x05 OUT, 0x09 PU, 0x13 DRV)。
...
ESP32-S3 は Bluetooth LE のみ (Classic 非対応) なので、BLE HID キーボードとして PC にペアリングし、ショートカットキーを送ります。
スタックは NimBLE-Arduino、実装は `src/hid.h`。
...
### 送るキー (`config.h` の 7. で変更可)

| 操作 | 既定のキー | 用途 |
|---|---|---|
| 会議モードのトグル (左 → 右 / 右 → 左、またはタップ) | **Alt+A** | Teams のミュート切替 (Teams のウィンドウにフォーカスがあること)。Windows 11 の通話ミュート Win+Alt+K は「対応アプリなし」になる環境があった |
| BtnA クリック | **Alt+Tab** | 直前のウィンドウと切り替え |
| BtnB クリック | **無変換** | 音声入力の起動に割り当てている想定。Windows の音声入力なら Win+H に変更可 |

- ミュート状態は **バッジ側で覚えているだけ**で PC からは返ってきません。PC 側で直接ミュートを操作するとズレるので、そのときはレバーをもう一度動かして合わせてください
- `HID_ENABLED = false` で BLE ごと無効化 (消費電力が少し減る)
