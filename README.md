...
| 画面右のレバー (PC 接続中のみ) | **マイクのミュート切替**。上 = MIC ON / 下 = MUTE。ドラッグかタップで切替、Alt+A (Teams) を送る |
...
| 操作 | 既定のキー | 用途 |
|---|---|---|
| レバー (上 → 下 / 下 → 上、またはタップ) | **Alt+A** | Teams のミュート切替 (Teams のウィンドウにフォーカスがあること)。Windows 11 の通話ミュート Win+Alt+K は「対応アプリなし」になる環境があった |
