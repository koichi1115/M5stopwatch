...
| 画面右のレバー (PC 接続中のみ) | **マイクのミュート切替**。上 = MIC ON / 下 = MUTE。ドラッグかタップで切替、Win+Alt+K を送る |
...
3. 接続されるとバッジが震えてステータスに `ble connected` と出て、画面右にミュートレバーが現れる
...
| 操作 | 既定のキー | 用途 |
|---|---|---|
| レバー (上 → 下 / 下 → 上、またはタップ) | **Win+Alt+K** | Windows 11 の通話ミュート切替。Teams 対応、Teams にフォーカスが無くても効く。Teams にフォーカスがある前提なら Ctrl+Shift+M に変更可 |
